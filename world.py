...
class World:
	def __init__(self, player, texture_manager, options):
		...
		layer_scale = 0.2
		layer_height = 5
		sample = [1] * 1000 + [16] * 10 + [15] * 5 + [14] * 3 + [56] * 1
		heights = []
		...
		for x in range(-size, size):
			...
			for z in range(-size, size):
				# evaluate noise function itself

				y = 64 + height * noise((x * scale, z * scale))
				y += layer_height * noise((x * layer_scale, z * layer_scale))
				z_heights.append(y)

				# create chunk if one containing the block position doesn't yet exist

				chunk_pos = get_chunk_position((x, y, z))

				if chunk_pos not in self.chunks:
					self.chunks[chunk_pos] = chunk.Chunk(self, chunk_pos)

				c = self.chunks[chunk_pos]
				lx, ly, lz = get_local_position((x, y, z))

				# by default, grass/dirt
				...
				c.blocks[lx][ly][lz] = grass

				# dirt layers underneath

				for i in range(1, 3):
					c.blocks[lx][ly - i][lz] = dirt

				# then, stone until we reach the bottom

				i = ly - 3

				while i > 0:
					c.blocks[lx][i][lz] = random.choice(sample)
					i -= 1

				# then, water until we reach y = 50
				...
				for i in range(-radius, radius + 1):
					for j in range(-radius, radius + 1):
						for k in range(-radius, radius + 1):
							...
							if dist <= radius:
								self.set_block((x + i, y + trunk + j, z + k), 18)

				for i in range(trunk):
					self.set_block((x, y + i, z), 17)
